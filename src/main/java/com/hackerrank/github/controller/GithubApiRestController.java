...
public class GithubApiRestController {
	...
	public ResponseEntity<?> getEventsById(@PathVariable("id") Long id) {
		...
		List<EventDTO> events = new ArrayList<>();
		iterable.forEach(event -> events.add(mapper.map(event, EventDTO.class)));
		List<EventDTO> res = events.stream().filter(e->e.getId().equals(id)).collect(Collectors.toList());
		
		return (res.size()>0)? ResponseEntity.ok().body(res):ResponseEntity.notFound().build() ;
	}
